...
class ContactForm7 {
	...
	public function sendsms_c7( $form ) {
        $this->log->add(ZMPLGXX_NAME, "Initiating Send SMS c7");
		...
		if ( $send_to_admin ) {
            $this->log->add(ZMPLGXX_NAME, "Sending SMS c7 to admin");
		...
		if ( $send_to_visitor ) {
            $this->log->add(ZMPLGXX_NAME, "Sending SMS c7 to visitor");
    ...
    public function new_menu_messaging( $panels ) {
		$panels['messaging-sms-panel'] = array(
		...
		if(!empty($_REQUEST['post'])){
			...
			if ( empty($options['visitor_mobile_field']) )
			{
				echo sprintf(
					...
					esc_html__( "Please choose mobile number field in MessagingAPI tab", MESSAGINGSMS_TEXT_DOMAIN)
				);
			}
		}
	}
